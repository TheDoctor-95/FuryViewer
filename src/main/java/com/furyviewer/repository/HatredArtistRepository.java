...
public interface HatredArtistRepository extends JpaRepository<HatredArtist, Long> {
    ...
    @Query("select h.artist from HatredArtist h where h.hated=true group by h.artist order by count (h.hated) desc")
    ...
    @Query("SELECT COUNT(h) FROM HatredArtist h WHERE h.hated=true AND h.user.login=:userLogin")
    Integer countUserArtistsHatred(@Param("userLogin") String userLogin);
}
